...
class SparseFactor {
 public:
  ...
  void print() const;
  // print all CHOLMOD attributes

  void build(SparseMatrix<T>& A, size_t nI = -1);
  // factorizes positive-definite matrix A using CHOLMOD
  // optionally specify a (positive) number of interior values for block
  // reordering

  void subfactor(size_t m, SparseFactor<double>& factor);
  // extracts the upper-left m x m sub-block, assuming that the
  // permutation used for the original factorization preserves
  // the first m rows/columns
  ...
  void subfactor_simplicial(size_t m, SparseFactor<double>& factor);
  void subfactor_supernodal(size_t m, SparseFactor<double>& factor);

  void printPermutation();
  void printOrderingMethod();
};
...
SparseMatrix<T>::SparseMatrix(size_t m_, size_t n_)
    : factors(*this), m(m_), n(n_), cData(NULL) {}

// copy constructor
template <typename T>
SparseMatrix<T>::SparseMatrix(const SparseMatrix<T>& B)
    : factors(*this), cData(NULL) {
  *this = B;
}
...
template <class T>
void SparseMatrix<T>::clearCompressed(void) {
  if (cData) {
...
template <class T>
void SparseMatrix<T>::buildCompressed(void) {
  clearCompressed();
  allocateSparse();

  // build compressed matrix (note that EntryMap stores entries in column-major
  // order)
